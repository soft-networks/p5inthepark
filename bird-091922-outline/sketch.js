...
function setup() {
  ...
  background(20, 5, 90, 100);
  frameRate(24);

  for (let i =0; i < 20; i++) {
    reedStore.push(makeReed());
  }
}
...
function draw() {
  ...
  background(20, 0, 100, 100);

  mRect(50,50, width-100, height-100, {GRAD_COL: [[20, 5,100]], OVERDRAW: 0})
...
const WING_COLOR = gg(40,60, 100, 100);
...
function body(move) {
  ...
  //facetw
  mEllipse(0, 0, 25, 27, gg(120,180,90,60), 20);
  ...
  translate(5, -5);
  mRect(0, 0, 20, 8, {...gg(50,100,100,100), ...fc(50)});
...
function reeds(s, e, p) {
  ...
  for (let t =e; t> s; t-= PI/32) {
    if (prn.rn() > ( p || 0.2)) {
      ...
      push();
      translate(x1,y1);
      mPoly(reedStore[i % reedStore.length], {GRAD_COL: [[120,80,100]]});
      pop();
      i++;
    }
...
function makeReed() {
  ...
  for (let i=0; i > r; i-= 15) {
    let l = map(i, 0, r, 1, 0.2);
    let b = dir  * Math.pow(i/r,2) * w;

    polyl.push(cv(-w * l + b, i));
    polyr.unshift(cv(w *l + b, i));
  } 
