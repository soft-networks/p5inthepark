...
//background(300, 100, 100);
  ...
  if (efc > regularFrames.length + flappingFrames.length)  {
    return;
  }

  if (efc >= regularFrames.length) {
    let lfc = efc - regularFrames.length;
    flappingFrames[lfc](); 
  } else {
    regularFrames[efc]();
  }
...
function saveFrame() {
  let d = new Date();
  console.log("saving");
  saveCanvas("park" + window.location.pathname + "_" + d.getHours() + "_" + SAVED_COUNT, "png");
  SAVED_COUNT++;
}
function animateFrames() {
  if (mouseIsPressed || false && dist(mouseX, mouseY, width/2, height/2) < ww/2) {
    isFlapping = true;
  } else {
    isFlapping = false;
  }
...
function fc(h, s= 100, b = 80) {
  return {FILL_COL: [h,s,b]};
}
const WING_COLOR = gg(40,60, 50, 90);
