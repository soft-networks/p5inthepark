...
const PREFS = {
  ...
  GRAD_COL: [[225,225,225], [50,225,50], [0,1]], //for now this always has to kinda be [0,1] because any other version doesnt really work
  OVERDRAW: 0,
};
...
function mEllipse(x0, y0, w, h, prefs ) {
  ...
  for (let t = 0; t < TAU; t += step) {
    ...
    strokeWeight(1);
    if (GRAD_COL) { 
      const segmentGrad = [GRAD_COL[0], GRAD_COL[1], [t/TAU, (t+step)/TAU]];
      mL(x1,y1,x2,y2, {GRAD_COL: segmentGrad});
    } 
    else {mL(x1, y1, x2, y2)};
  }
}
