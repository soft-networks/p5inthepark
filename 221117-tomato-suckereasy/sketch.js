...
const BRANCH_PROB = (node) => {
  ...
  if (depth == 1 && node.index >= 1 ) return 1.0;
  if (depth == 2) {
...
const END_REACHED = (node) => {
  ...
  if (node.sucker && node.sucker >=2 && node.index > 20) return true;
  if (depth == 0) return node.y < 0 || index > rnd(25, 33);
...
const CURVE_ANGLE = (node) => {
  ...
  let d = map(node.depth, 0, 3, 0.1, 1);
  return c *  d *rnd(0.8,1.2);
};
  ...
  produceNodes.forEach((produceNode) => {
    ...
    if (produceNode.type == "fruit") {
      ...
      mEllipse(miniLeafPoint.x, miniLeafPoint.y, 2, 3, {
        foregroundFill: PALETTE.miniLeaf,
      ...
      mPoly(leafToPoly(producePoint, produceNode), {
        strokeFill: [produceNode.color[0], 30, 70, 100],
        foregroundFill: [produceNode.color[0], 50,  40,  20],
      });
    }
...
function growNode(node) {
  ...
  if (nextNode.depth == 0) {
    ...
    nextNode.angle +=  (nextNode.angle > -PI / 2 ? 1 : -1) * CURVE_ANGLE(node); // TODO
  } else {
...
function branchNode(node, branchNum) {
  ...
  let shouldBecomeSucker =
    node.depth == 0 && (!node.sucker || node.sucker < 2) &&  Math.random() < map(node.index, 1, 10, 0, 0.5) ;
  let nextNode = {
    index: 0,
    depth: nextDepth,
    x: node.x,
  ...
  if (shouldBecomeSucker) {
    ...
    nextNode.thickness = node.thickness;
    nextNode.sucker = 1;

    let flip = fliprn();
    let suckerAngle = Math.PI / 6;
    nextNode.angle = node.angle + flip * rnd(0.8, 1.2) * suckerAngle;

    node.thickness = node.thickness * 0.8;
    node.angle += -1 * flip * suckerAngle * rnd(0.2, 0.5);
    node.sucker = (node.sucker || 1) + 1;
