...
let curveAngle = 0.01;
let thicknessStep = 0.01;
let thicknessJump = 0.8;
let stemLengthStep = 0.1;

const BRANCH_PROB = (depth, index) => {
  if (depth > 0 && index == 0) return 0;
  else return 0.07 * (depth/2 + 1);
};
const PRODUCE_PROB = 0.5;
const FRUIT_PROB = 0.2;
/**
...
function growNode(node) {
  ...
  nextNode.y = nextP.y;
  nextNode.index += 1;
  nextNode.angle += (nextNode.flipped ? 1 : -1) * curveAngle;
...
function branchNode(node) {
  let nextNode = {
    ...
    y: node.y,
    height: node.height,
    thickness: node.thickness * thicknessJump,
  };
  let flip = fliprn() > 0; 
  nextNode.angle = flip ? node.angle + PI/4 : node.angle - PI/4;
  nextNode.flipped = flip;
